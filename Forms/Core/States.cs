...
public class States {
	...
	internal bool Undo() {
		if (Suppressed || _undo.Count < 1 || _undo.Pop() is not { } s) return false;
		Suppressed = true;
		try {
			if (s.TryUndoFailed()) { Suppressed = false; return false; }
			_redo.Push(s);
...
internal abstract class LogSet<T>(States t) : ILogSet { // Every child calls LogUndo after binding its controls, to record the initial state
	...
	static protected void SetText(RichTextBox b, string t) {
		if (b.Text != t)
			b.Text = t;
	}
}
...
internal class LogAni : LogSet<AniState> { // triple textbox pinnable range
	...
	override protected void RestoreRedo(LogState<AniState> _, LogState<AniState> to) {
		SetText(_l, to.state.l);
		SetText(_f, to.state.f);
...
internal class LogText : LogSet<string> { // for a single code box
	...
	override protected void RestoreRedo(LogState<string> _, LogState<string> to) => SetText(_text, to.state); // it will trigger textChanged which will reparse and recolor the box, and tries to push undo which will be blocked
	override protected string SerializeV()  {
		var s = Undo.Peek();
		return s.state + "—";
	}
...
internal class LogOutput : LogSet<OutState> { 
	...
	override protected void RestoreUndo(LogState<OutState> from, LogState<OutState> to) {
		...
		switch ((OutputAction)from.action) {
			case OutputAction.Add:
				SetCombo(_s.outputSelect, from.state.s);
				_c.Del(_s.outputSelect, EventArgs.Empty);
				break;
			case OutputAction.Remove:
				SetCombo(_s.outputSelect, to.state.s);// re-add it back
	...
	private static void SetCombo(ComboBox c, string t) {
		if(c.Text != t)
			c.Text = t;
	}
	private void LoadBack(OutState s) {
		_s.outputSelect.SelectedItem = s.s;
		SetText(_s.rgbBox, s.r);
		SetText(_s.codeBox, s.c);
