...
public partial class PlotControlO : ParentControl {
	...
	public PlotControlO() => InitializeComponent();
	public PlotControlO(MenuControl? root, ParentForm parent) : base(root, parent) {
		InitializeComponent();
		...
		if (p is null)
			return;
		_fixedY = new(fyInputBox, FixedY);
		InputX = new(context, ixsInputBox, ixcInputBox, ixeInputBox, p.GetAxis()[0]);
		InputY =  new(context, iysInputBox, iycInputBox, iyeInputBox, p.GetAxis()[1]);
		InputT =  new(context, itsInputBox, itcInputBox, iteInputBox, p.GetAxis()[2]);
		OutputY =  new(context, oysInputBox, oycInputBox, oyeInputBox, p.GetAxis()[3]);
